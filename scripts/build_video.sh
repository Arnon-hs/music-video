...
REPORT="$ROOT/metadata/build-report.txt"
FORCE=0
[ "${1:-}" = "--force" ] && FORCE=1
...
mkdir -p "$ROOT/output" "$ROOT/metadata"
...
{
  echo "label=NON_COMMERCIAL_DEMO"
  echo "output=$OUT"
  echo "duration_seconds=$DURATION"
  echo "size_bytes=$(stat -f%z "$OUT")"
  echo "video=H.264, 1920x1080, 24 fps, yuv420p"
  echo "audio=AAC, 320 kbps"
  echo "music_license=MusicGen CC-BY-NC 4.0; replace the MusicGen WAV before commercial release."
...
echo "Created $OUT — NON_COMMERCIAL_DEMO"
echo "Report: $REPORT"
