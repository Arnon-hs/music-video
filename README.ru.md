...
9. не коммитить `.env`, сгенерированные медиа, модели, checkpoint’ы и логи; проверенные скриншоты документации разрешены только в `docs/images`;
...
- английский интерфейс по умолчанию и запоминаемый переключатель `EN`, `RU`, `简中`.
...
### Как выглядит dashboard и CLI

Dashboard открывается на английском. Кнопки в правом верхнем углу переключают его на русский или упрощённый китайский; браузер запоминает выбор.

![Английский web dashboard во время рендера часового плейлиста](docs/images/web-dashboard-en.png)

Посмотреть доступные инструментальные жанры:

![Пример списка музыкальных жанров в CLI](docs/images/cli-genres.png)

Проверить план часового плейлиста без загрузки модели и генерации файлов:

![Пример dry-run часового lo-fi плейлиста в CLI](docs/images/cli-playlist-dry-run.png)
...
- не добавляйте сгенерированные медиа, модели, сторонние checkout’ы и credentials; проверенные скриншоты документации храните только в `docs/images`;
