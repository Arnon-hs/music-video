...
- Начните с 30–60 секунд. Успешный `doctor` подтверждает пути, но не скорость, запас памяти или качество музыки.
- На Apple Silicon с 16 GB запускайте один backend за раз и оставляйте свободное место и на диске проекта, и на системном томе macOS. CPU fallback заметно медленнее.
- Для арендуемой NVIDIA GPU класс 24 GB VRAM — консервативная стартовая точка, а не гарантированный минимум. Меньшая GPU может заработать с offload/quantization, но проект не включает все upstream-оптимизации автоматически.
- CPU-only возможен у части backend’ов, однако часовой плейлист состоит из многих независимых генераций и может занять очень долгое время.
- На постоянном диске должны поместиться репозиторий, Python-окружение, веса, кэш, отдельные треки, временное аудио и финальный MP4.

Устанавливайте точную версию модели по upstream-инструкции, затем снова запускайте `./music-video doctor`. Новая upstream-версия не обязательно совместима с текущим адаптером.
